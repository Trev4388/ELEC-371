...
#define SWITCH_0 ((volatile unsigned int *) 0x10000040) //switch 0
...
#define SEGMENTS_CONTROL ((volatile unsigned int *) 0x10000020)
...
#define JTAG_DATA ((volatile unsigned int *) 0x10001000)
#define JTAG_CONTROL ((volatile unsigned int *) 0x10001004)
...
void interrupt_handler(void)
{
   ...
   if (ipending & 3){
	   ...
	   if(s1==0){
	   s1=1;
		*SEGMENTS_CONTROL = 1886417008;
		   
	   }else if(s1==1){
	   s1=0;
		*SEGMENTS_CONTROL =1179010630;
	   }	
	   // NOTE: This uses buttons 0 and 1. This works on CPULator but may not on real hardware
...
void Init (void)
{
   ...
   *TIMER_START_HI = 0x00BE; //0.25 sec
   *TIMER_STATUS = 0;
   *TIMER_CONTROL = 7;//0b0111;
	
	//timer for blinker leds
   *T0_START_LO = 0x9680;
   *T0_START_HI = 0x0098; //0.2 sec
   *T0_STATUS = 0;
   *T0_CONTROL = 7;//0b0111;
...
int main (void)
{
	...
	while (1)
	{
		/* fill in body of infinite loop */
     	if(charToPrint){
        // The anding then oring might be overkill, we can probably just write straight to the location
        //*JTAG_DATA = (*JTAG_DATA & 0xFFFFFF00) | charToPrint;
		PrintChar('\b');
		int x =(*SWITCH_0 & 0x00000001);
		if(x == 1){
			PrintChar('1');
		}else{
			PrintChar('0');
